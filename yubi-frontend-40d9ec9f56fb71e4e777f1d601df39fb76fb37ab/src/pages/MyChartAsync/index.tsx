                        ...
                        {item.status === 'success' ? (
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
                            <span style={{ fontSize: 16, color: '#999' }}>
                              {item.status === 'wait' ? '图表正在等待处理...' : 
                               item.status === 'running' ? '图表正在生成中...' : 
                               '图表生成失败'}
                            </span>
                          </div>
                        )}
              ...
              <List.Item.Meta
                      ...
                      {item.status && (
                        <span 
                          style={{
                            ...
                            fontSize: 12,
                            fontWeight: 'normal',
                            backgroundColor: item.status === 'wait' ? '#fff2e8' : 
                                            item.status === 'running' ? '#e6f7ff' : 
                                            item.status === 'success' ? '#f6ffed' : 
                                            '#fff1f0',
                            color: item.status === 'wait' ? '#fa8c16' : 
                                   item.status === 'running' ? '#1890ff' : 
                                   item.status === 'success' ? '#52c41a' : 
                                   '#ff4d4f',
                          }}
                        >
                          {item.status === 'wait' ? '等待中' : 
                ...
                description={
                  <div>
                    ...
                    {item.status && item.status === 'failed' && item.execMessage && (
                      <div style={{ color: '#ff4d4f' }}>失败原因：{item.execMessage}</div>
                    )}
                    {item.createTime && (
